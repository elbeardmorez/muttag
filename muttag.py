        ...
        for frame in tag.getall('APIC'):
            ...
            if os.path.exists(fPathFile):
                ...
                while os.path.exists(
                          fPath + os.sep + fName + str(l) + u'.' + fType):
                    l += 1
            ...
            if l > 0:
                if filecmp.cmp(
                    ...
                    for ll in range(2, l):
                        if filecmp.cmp(fPathFile,
                                       fPath + os.sep + fName + str(ll) +
                                       u'.' + fType):
                            log(u'[debug] deleting duplicate image')
                            os.remove(fPathFile)
                            break
...
def optionsListCallback(option, opt, value, parser):
    if value == "":
        ...
        setattr(optionParser.values, option.dest,
                map(lambda s: string.replace(s.rstrip(","), "\\,", ","),
                    re.findall("(.*?[^\\\],|.+$)", value)))


def optionsPathExpansionCallback(option, opt, value, parser):
    setattr(optionParser.values, option.dest,
            os.path.expandvars(os.path.expanduser(value).split('|')))

optionParser.add_option(
    "-d", "--dir", metavar="DIR", type="string", dest="pathRoot",
    default=os.path.curdir, action="callback",
    callback=optionsPathExpansionCallback,
    help="set DIR to the root music directory")
optionParser.add_option(
    "-f", "--files", metavar="FILES", type="string", dest="pathFiles",
    default="", action="callback",
    callback=optionsPathExpansionCallback,
    help="comma-delimited list of absolute or "
         "'pathRoot'-relative music files/paths")
optionParser.add_option(
    "-a", "--extract-art", dest="extractart", default=False,
    action="store_true",
    help="extract album art [mp3 support only]"),
optionParser.add_option(
    "-o", "--output-info", metavar="OUTPUT", type="string", dest="output",
    default="./muttag.info",
    help="set OUTPUT as target to dump all tag info to")
optionParser.add_option(
    "-x", "--extensions", metavar="EXTENSIONS", type="string",
    dest="extensions", default=(".mp3", ".ogg"), action="callback",
    callback=optionsListCallback,
    help="EXTENSIONS is a comma delimited list of "
         "supported file types")
optionParser.add_option(
    "-v", "--verbosity", metavar="VERBOSITY", type="int",
    dest="verbosity", default=1, action="store",
    help="log level. log issues and a subset of tag "
         "info to log file (defaulting to stdout)")

options, args = optionParser.parse_args()  # options (dict), args (list)
    ...
    try:
        for path in list:
            if not os.path.exists(path) and \
                   os.path.exists(options.pathRoot[0] + os.path.sep + path):
            ...
            if os.path.isfile(path):
                ...
                for root, subdir, files in os.walk(path):
                    if len(files) > 0:
                        ...
                        header = u'location: ' + \
                                 os.path.abspath(root).decode('utf-8')
